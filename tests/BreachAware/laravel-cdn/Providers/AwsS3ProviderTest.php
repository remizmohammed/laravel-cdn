...
class AwsS3ProviderTest extends TestCase
{
    public function setUp()
    {
        ...
        $this->m_validator = M::mock('BreachAware\LaravelCdn\Validators\Contracts\ProviderValidatorInterface');
        $this->m_validator->shouldReceive('validate');

        $this->m_helper = M::mock('BreachAware\LaravelCdn\CdnHelper');
        $this->m_helper->shouldReceive('parseUrl')
            ->andReturn($this->pased_url);

        $this->m_spl_file = M::mock('Symfony\Component\Finder\SplFileInfo');
        $this->m_spl_file->shouldReceive('getPathname')->andReturn('BreachAware/laravelcdn/tests/BreachAware/laravelcdn/AwsS3ProviderTest.php');
        $this->m_spl_file->shouldReceive('getRealPath')->andReturn(__DIR__.'/AwsS3ProviderTest.php');

        $this->p_awsS3Provider = M::mock('\BreachAware\LaravelCdn\Providers\AwsS3Provider[connect]', array(
            $this->m_console,
            $this->m_validator,
            $this->m_helper,
        ));
    ...
    public function testInitializingObject()
    {
        ...
        assertInstanceOf('BreachAware\LaravelCdn\Providers\AwsS3Provider', $awsS3Provider_obj);
    }
