...
namespace BreachAware\LaravelCdn\Tests;
...
class CdnTest extends TestCase
{
    ...
    {
        $this->m_asset = M::mock('BreachAware\LaravelCdn\Contracts\AssetInterface');
        ...
        $this->m_finder = M::mock('BreachAware\LaravelCdn\Contracts\FinderInterface');
        $this->m_finder->shouldReceive('read')
            ->with($this->m_asset)
            ->once()
            ->andReturn(new Collection());

        $this->m_provider = M::mock('BreachAware\LaravelCdn\Providers\Provider');
        $this->m_provider->shouldReceive('upload')
            ->once()
            ->andReturn(true);

        $this->m_provider_factory = M::mock('BreachAware\LaravelCdn\Contracts\ProviderFactoryInterface');
        $this->m_provider_factory->shouldReceive('create')
            ->once()
            ->andReturn($this->m_provider);

        $this->m_helper = M::mock('BreachAware\LaravelCdn\Contracts\CdnHelperInterface');
        $this->m_helper->shouldReceive('getConfigurations')
            ->once()
            ->andReturn([]);

        $this->cdn = new \BreachAware\LaravelCdn\Cdn(
            $this->m_finder,
            $this->m_asset,
    ...
    public function testPushCommand()
    {
        ...
        $finder = new \BreachAware\LaravelCdn\Finder($m_consol);

        $asset = new \BreachAware\LaravelCdn\Asset();

        $provider_factory = new \BreachAware\LaravelCdn\ProviderFactory();
        ...
        $helper = new \BreachAware\LaravelCdn\CdnHelper($m_config);

        $m_console = M::mock('Symfony\Component\Console\Output\ConsoleOutput');
        $m_console->shouldReceive('writeln')
            ->atLeast(2);

        $m_validator = M::mock('BreachAware\LaravelCdn\Validators\Contracts\ProviderValidatorInterface');
        $m_validator->shouldReceive('validate');

        $m_helper = M::mock('BreachAware\LaravelCdn\CdnHelper');

        $m_spl_file = M::mock('Symfony\Component\Finder\SplFileInfo');
        $m_spl_file->shouldReceive('getPathname')
            ->andReturn('BreachAware\laravel-cdn/tests/BreachAware/laravelcdn/AwsS3ProviderTest.php');
        $m_spl_file->shouldReceive('getRealPath')
            ->andReturn(__DIR__.'/AwsS3ProviderTest.php');

        // partial mock
        $p_aws_s3_provider = M::mock('\BreachAware\LaravelCdn\Providers\AwsS3Provider[connect]', 
        [
            $m_console,
            $m_validator,
            $m_helper,
        ...
        $cdn = new \BreachAware\LaravelCdn\Cdn($finder,
            $asset,
            $provider_factory,
            $helper
        );
